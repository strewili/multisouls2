...
namespace Valve.VR.InteractionSystem
{
	...
	public class Throwable : MonoBehaviour
	{
        ...
        public ReleaseStyleVR releaseVelocityStyle = ReleaseStyleVR.GetFromHand;
        ...
        {
            if (hand.noSteamVRFallbackCamera && releaseVelocityStyle != ReleaseStyleVR.NoChange)
                releaseVelocityStyle = ReleaseStyleVR.ShortEstimation; // only type that works with fallback hand is short estimation.

            switch (releaseVelocityStyle)
            {
                case ReleaseStyleVR.ShortEstimation:
                    velocityEstimator.FinishEstimatingVelocity();
                    velocity = velocityEstimator.GetVelocityEstimate();
                    angularVelocity = velocityEstimator.GetAngularVelocityEstimate();
                    break;
                case ReleaseStyleVR.AdvancedEstimation:
                    hand.GetEstimatedPeakVelocities(out velocity, out angularVelocity);
                    break;
                case ReleaseStyleVR.GetFromHand:
                    velocity = hand.GetTrackedObjectVelocity(releaseVelocityTimeOffset);
                    angularVelocity = hand.GetTrackedObjectAngularVelocity(releaseVelocityTimeOffset);
                    break;
                default:
                case ReleaseStyleVR.NoChange:
                    velocity = rigidbody.linearVelocity;
                    angularVelocity = rigidbody.angularVelocity;
                    break;
            }

            if (releaseVelocityStyle != ReleaseStyleVR.NoChange)
